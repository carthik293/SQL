...
select BD.BIDDER_NAME,count(*) AS 'Total number of Bids by Individual',
sum(case BID.BID_STATUS when 'won' then 1 else 0 end) as 'Total Number of matches Won by Individual', 
(sum(case BID.BID_STATUS when 'won' then 1 else 0 end)/count(*))*100 as '%age of Wining by Individual'
from ipl_bidder_details BD
join ipl_bidding_details BID
on BD.BIDDER_ID = BID.BIDDER_ID
group by BD.BIDDER_NAME,BID.BIDDER_ID
ORDER BY (sum(case BID.BID_STATUS when 'won' then 1 else 0 end)/count(*))*100 DESC;

#2 Display the number of matches conducted at each stadium with stadium name, city from the database.

select ISD.STADIUM_NAME,ISD.CITY,count(*) AS 'No of matched conducted'
from ipl_match_schedule IPS
join ipl_stadium ISD
on IPS.STADIUM_ID = ISD.STADIUM_ID
group by ISD.STADIUM_ID;

#3. In a given stadium, what is the percentage of wins by a team which has won the toss

select ISD.STADIUM_NAME,
(sum(case when IM.TOSS_WINNER = IM.MATCH_WINNER then 1 else 0 end)/count(*)) * 100 as '%age Team wining both the Toss and the Match',
count(*) as 'Total Number of Matches'
from ipl_match_schedule IMS
join ipl_match IM
on IMS.MATCH_ID = IM.MATCH_ID
join ipl_stadium ISD
on IMS.STADIUM_ID = ISD.STADIUM_ID
group by ISD.STADIUM_ID;

#4. Show the total bids along with bid team and team name

select count(NO_OF_BIDS) AS 'Total Bids',IBID.BID_TEAM,IT.TEAM_NAME
from ipl_bidder_details IBED
join ipl_bidding_details IBID
on IBED.BIDDER_ID = IBID.BIDDER_ID
join ipl_bidder_points IBP
on IBED.BIDDER_ID = IBP.BIDDER_ID
join ipl_match_schedule IMS
on IBID.SCHEDULE_ID = IMS.SCHEDULE_ID
join ipl_match IM
on IMS.MATCH_ID = IM.MATCH_ID
join ipl_team IT
on IM.TEAM_ID1 = IT.TEAM_ID
group by IBP.BIDDER_ID,IT.TEAM_NAME;

# 5. Show the team id who won the match as per the win details

select IM.MATCH_ID,IM.WIN_DETAILS,
case
when IM.MATCH_WINNER = 1 then TEAM_ID1 else TEAM_ID2
end as 'Team ID'
from ipl_match IM
...
select sum(MATCHES_PLAYED),sum(MATCHES_WON),sum(MATCHES_LOST),IT.TEAM_NAME
from ipl_team IT
...
select IP.PLAYER_NAME,IP.PLAYER_ID,ITP.PLAYER_ROLE
from ipl_team_players ITP
join ipl_player IP
on ITP.PLAYER_ID = IP.PLAYER_ID
where (ITP.PLAYER_ROLE = 'Bowler') and (ITP.TEAM_ID = 5);

#8. 8.How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order

select ITP.PLAYER_ROLE,count(*) as 'No of All-Rounders',IT.TEAM_NAME
from ipl_team_players ITP
join ipl_player IP
on ITP.PLAYER_ID = IP.PLAYER_ID
join ipl_team IT
